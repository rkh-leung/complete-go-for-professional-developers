...
type WorkoutStore interface {
	...
	// Recommend to always add function definition first
...
func (pg *PostgresWorkoutStore) GetWorkoutByID(id int64) (*Workout, error) {
	...
	WHERE id = $1
	`
	err := pg.db.QueryRow(query, id).Scan(&workout.ID, &workout.Title, &workout.Description, &workout.DurationMinutes, &workout.CaloriesBurned) // returns at most one row
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
  ...
  SET title = $1, description = $2, duration_minutes = $3, calories_burned = $4
	...
	rowsaffected, err := result.RowsAffected() // supported by postgres & pgx
	if err != nil {
		return err
	}
	if rowsaffected == 0 {
